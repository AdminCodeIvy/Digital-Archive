  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (!acceptedFileTypes.includes(file.type)) {
        setUploadError('File type not supported. Please upload PDF or image files only.');
        toast.error('File type not supported. Please upload PDF or image files only.');
        return;
      }
      ...
      try {
        ...
        if (documentData && documentData.secure_url) {
          ...
          if (response.data && response.data[0] && response.data[0].id) {
            ...
            setTimeout(() => {
              console.log(`Redirecting to /scanner/documents/${documentId}`);
              navigate(`/scanner/documents/${documentId}`);
            }, 1000);
          } else {
        ...
        <div>
          <div className="flex items-center gap-2">
            <Button 
              ...
              size="icon" 
              className="h-8 w-8"
              onClick={() => navigate('/scanner/documents')}
            >
