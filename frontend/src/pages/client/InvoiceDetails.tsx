...
const InvoiceDetailsClient = () => {
  ...
  const submitInvoiceMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      refetchData().then(() => {
        navigate(`/${userRole}/current-invoices`);
      });
    },
    onError: () => {
  ...
  if (!invoice) {
    return (
      <div className="text-center py-10">
        ...
        <Button onClick={() => navigate(`/${userRole}/current-invoices`)}>Back to Invoices</Button>
      </div>
    );
  }
  ...
  const getStatusBadge = () => {
    if (invoice.invoice_submitted_admin) {
      return <Badge className="bg-green-100 text-green-800">Owner Verified</Badge>;
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          <Button 
            variant="outline" 
            size="sm" 
            onClick={() => navigate(`/${userRole}/current-invoices`)}
          >
