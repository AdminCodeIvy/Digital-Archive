  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0) {
      ...
      if (!acceptedFileTypes.includes(file.type)) {
        ...
        toast.error('File type not supported. Please upload PDF only.');
        return;
      }
      ...
      try {
        ...
        if (documentData && documentData.secure_url) {
          ...
          const data = {
            ...
            tag_id: selectedTag,
            tag_name: tagName,
            file_id: '282', // Note the client uses a different file_id
            title: documentTitle
          };
          ...
          if (response.data && response.data[0] && response.data[0].id) {
            ...
            setTimeout(() => {
              console.log(`Redirecting to /client/documents/${documentId}`);
