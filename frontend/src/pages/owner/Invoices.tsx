...
import { Loader2, FileText, Search, RefreshCw, Plus } from 'lucide-react';
...
const CompanyInvoice = () => {
  ...
  const { data: invoiceSubmissionData } = useQuery({
  ...
  const submitAllInvoicesMutation = useMutation({
    ...
    onError: (error) => {
      toast.error('Failed to submit company invoices');
    }
  });
              ...
              {filteredInvoices.length > 0 ? (
                filteredInvoices.map((invoice: Invoice) => (
                  <TableRow key={invoice.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                    ${parseFloat(invoice.invoice_value || invoice.amount || 0).toFixed(2)}
                    </TableCell>
                    <TableCell>
