...
import { ArrowRight, Filter as FilterIcon, Folder, Search as SearchIcon } from 'lucide-react';
...
const DocumentDashboard = () => {
  ...
  const filteredDocuments = documents.filter(doc => {
    // Apply search filter
    if (searchQuery && !((doc.title && doc.title.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (doc.tag_name && doc.tag_name.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (doc.properties && doc.properties.some((prop: any) =>
        (prop.name && prop.name.toString().toLowerCase().includes(searchQuery.toLowerCase())) 
      ||
        (prop.value && prop.value.toString().toLowerCase().includes(searchQuery.toLowerCase()))
      )))) {
      return false;
    }
    ...
    if (activeFilters.status && activeFilters.status !== 'all') {
      ...
      const filterStatus = activeFilters.status.charAt(0).toUpperCase() + activeFilters.status.slice(1);
    ...
    if (activeFilters.role && activeFilters.role !== 'all') {
      if (!doc.requested_by?.role || doc.requested_by.role.toLowerCase() !== activeFilters.role.toLowerCase()) {
        return false;
      }
    }
    ...
    if (activeFilters.startDate) {
      ...
      const filterDate = new Date(activeFilters.startDate);
    ...
    if (activeFilters.endDate) {
      const docDate = new Date(doc.created_at);
      const filterDate = new Date(activeFilters.endDate);
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="overflow-hidden cursor-pointer hover:shadow-md transition-all" onClick={() => navigate('/owner/documents/tags')}>
        ...
        <Card 
          ...
          onClick={() => navigate('/owner/documents/filtered/incomplete')}
        >
          <CardContent className="p-6">
            <div className="flex flex-col">
        ...
        <Card 
          className="overflow-hidden cursor-pointer hover:shadow-md transition-all"
          onClick={() => navigate('/owner/documents/filtered/complete')}
        >
          <CardContent className="p-6">
            <div className="flex flex-col">
        ...
        <Card className="overflow-hidden cursor-pointer hover:shadow-md transition-all bg-orange-500 text-white"
          onClick={() => navigate('/owner/documents/scan')}>
          <CardContent className="p-6">
            <div className="flex flex-col">
              ...
              {filteredDocuments.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8 text-muted-foreground">
                    {searchQuery || activeFilters.status !== 'all' || activeFilters.role !== 'all' || activeFilters.startDate || activeFilters.endDate 
                      ? 'No documents match your search criteria and filters' 
                      : 'No documents found'}
                  </TableCell>
                </TableRow>
              ) : (
                filteredDocuments.map((doc, index) => {
                  ...
                  return (
                    <TableRow key={index}>
                      ...
                      <TableCell>{doc.title || '_'}</TableCell>
                      ...
                      <TableCell className="text-right">
                        {status === 'Incomplete' ? (
                          <Button 
                            ...
                            onClick={() => navigate(`/owner/documents/${doc.id || '12345'}`)}
                          >
                            Continue Editing
                          </Button>
                        ) : (
