...
export default function ManagerDashboard() {
  ...
  const { data: clientMetrics, isLoading: isLoadingClientMetrics } = useQuery({
    queryKey: ['clientOverviewMetrics'],
  ...
  // Calculate total users count safely
  const totalUsers = usersData?.data?.length || 0;
  
  // Get client invoice data safely
  const totalInvoiceValue = clientMetrics?.data?.totalInvoiceValue || "0.00";
  const totalPaidInvoices = clientMetrics?.data?.totalInvoicesPaid || 0;
  const totalDocumentsDownloaded = clientMetrics?.data?.totalDocumentsDownloaded || 0;
  ...
  const isLoading = isLoadingStats || isLoadingDocs || isLoadingClientMetrics || isLoadingUsers || disputesLoading;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
        {/* <StatsCard
          ...
          onClick={() => navigate('/manager/client-invoices')}
        />
