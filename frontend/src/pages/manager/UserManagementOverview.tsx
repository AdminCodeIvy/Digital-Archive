...
const UserManagementOverview = () => {
  ...
  const { data: usersData, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Ensure we always return an array, filter out owner users, and handle null data
      const users = Array.isArray(response.data) ? response.data as UserType[] : [];
      return users.filter(user => user.role.toLowerCase() !== 'owner');
    }
  });

  // Safely filter users by ensuring usersData is an array before filtering
  ...
  // Calculate counts with null safety checks
  ...
  const handleDeleteUser = async (userId: string) => {
    if (window.confirm('Are you sure you want to delete this user? This action cannot be undone.')) {
      try {
        ...
        // Since 204 means success with no content, we can show success message
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card className="overflow-hidden cursor-pointer hover:shadow-md transition-all">
          <CardContent className="p-6">
            <div className="flex flex-col">
              <p className="text-sm font-medium text-muted-foreground mb-1">Scanners</p>
              <h3 className="text-3xl font-bold">{scanners}</h3>
            </div>
            <div className="mt-4 flex items-center text-xs text-orange-500">
              <span>View details</span>
              <ArrowRight className="ml-1 h-3 w-3" />
        ...
        <Card className="overflow-hidden cursor-pointer hover:shadow-md transition-all">
          <CardContent className="p-6">
            <div className="flex flex-col">
              <p className="text-sm font-medium text-muted-foreground mb-1">QA Users</p>
              <h3 className="text-3xl font-bold">{qaUsers}</h3>
            </div>
            <div className="mt-4 flex items-center text-xs text-orange-500">
              <span>View details</span>
              <ArrowRight className="ml-1 h-3 w-3" />
        ...
        <Card className="overflow-hidden cursor-pointer hover:shadow-md transition-all bg-orange-500 text-white" onClick={() => navigate('/manager/users/create')}>
          <CardContent className="p-6">
            <div className="flex flex-col">
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">Documents Reviewed</TableHead>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.slice(0, 7).map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-center">{user.documents_reviewed}</TableCell>
