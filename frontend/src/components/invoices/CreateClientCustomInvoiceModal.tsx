...
import { fetchClients, createCustomInvoice, updateCustomInvoice, fetchCustomInvoice } from '@/services/api';
import { CustomInvoice, InvoiceQuantity } from '@/types';
import { DatePicker } from '@/components/ui/date-picker';

interface CreateClientCustomInvoiceModalProps {
  isOpen: boolean;
  onClose: () => void;
  invoiceId?: string;
  onSuccess?: () => void;
...
export const CreateClientCustomInvoiceModal: React.FC<CreateClientCustomInvoiceModalProps> = ({
  isOpen,
  onClose,
  invoiceId,
  onSuccess,
  ...
  const handleClientChange = (clientId: string, clientName: string) => {
    if (clientsData?.data) {
      ...
      handleInputChange('user_id', clientId);
    }
  };

  const handleSubmit = () => {
    // Validate required fields
    if (!invoice.client_name || !invoice.date || !invoice.due_date || !invoice.payment_term) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {(isLoadingInvoice || isLoadingClients) ? (
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      const selectedClient = clientsData?.data?.find((client: any) => client.id === value);
                      handleClientChange(value, selectedClient?.name || '');
                    }}
                  >
...
export default CreateClientCustomInvoiceModal;
